...
def log_in():
    with sentry_sdk.start_transaction( name="log_in", op="log_in"):
...
def erneut_spielen():
    with sentry_sdk.start_transaction( name="erneut_spielen", op="erneut_spielen"):
...
def main():
    with sentry_sdk.start_transaction( name="main", op="main"):
        # is the OS Windows? Then log in
        if os.name == "nt":
            sentry_sdk.add_breadcrumb(category="info", message="Windows OS")
            sentry_sdk.set_context("os", {"type": "Windows"})
            log_in()
        else:
            sentry_sdk.add_breadcrumb(category="info", message="Other OS type (not Windows)")
            sentry_sdk.set_context("os", {"type": "Other"})
